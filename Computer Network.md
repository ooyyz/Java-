...
- 应用层：HTTP和HTTPS
    ...
    - [除了GET和POST还有哪些方法](#除了GET和POST还有哪些方法)
...
4. 如果证书受信任，或者用户接收了不受信任的证书，浏览器会生成一个**随机密钥**（用于对称算法），并用服务器提供的公钥加密（采用非对称算法对密钥加密）；**使用Hash算法对握手消息进行摘要计算**，并用密钥对摘要值加密；将加密后的随机密钥和摘要一起发送给服务器；
...
- 摘要算法:MD5、SHA
- SHA各方面优于MD5算法，现在广泛应用SHA-2算法，安全性很高
</details>
